...
class SoundBoardHelp extends Application {
    ...
    _onResize() {
        super._onResize();
        const ytHelpEl = $('.yt-help-video');
        const sbHelpEl = $('.soundboard-help');
        ytHelpEl.width(sbHelpEl.width());
        ytHelpEl.height((sbHelpEl.width() / 16) * 9);
    ...
    getData() {
        let helpItems = [{
            title: 'General Usage', body: `<p><b>SoundBoard by Blitz</b> is designed to enhance and simplify on-the-fly sound effects for a GM</p>
            <b>Features</b>
            <ul>
